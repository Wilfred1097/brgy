...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        // Get the Sub Program ID from the AJAX request
        $programId = isset($_POST['id']) ? intval($_POST['id']) : 0;

        if ($programId > 0) {
            // Prepare SQL DELETE query
            $sql = "DELETE FROM sub_program WHERE id = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':id', $programId, PDO::PARAM_INT);

            // Execute the query
            if ($stmt->execute()) {
                echo json_encode(["status" => "success", "message" => "Sub Program successfully deleted!"]);
            } else {
